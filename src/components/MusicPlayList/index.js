...
class MusicPlayList extends Component {
  state = {playList: initialTracksList, searchInput: ''}
  ...
  renderNoResults = () => (
    <div className="no-found">
      <p className="no-found-paragraph">No Songs Found</p>
    </div>
  )

  onChangePlayItemsList = id => {
    const {playList} = this.state
    const upDatedList = playList.filter(eachItem => eachItem.id !== id)
    console.log(id)
  ...
  renderPlayList = () => {
    ...
    const searchResults = playList.filter(eachSong =>
    ...
    return (
      <ul className="playlist-container">
        {searchResultsLength === 0
          ...
          : searchResults.map(eachPlayList => (
              <PlayItems
                key={eachPlayList.id}
                eachPlayList={eachPlayList}
                playItemsList={this.onChangePlayItemsList}
              />
            ))}
      </ul>
    )
